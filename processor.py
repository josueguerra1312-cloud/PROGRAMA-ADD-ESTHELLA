...
def _ocr_items(engine, image):
    result = engine(image)
    # RapidOCR 3.x returns an object; older releases may return (result, elapsed).
    if isinstance(result, tuple): result = result[0]
    items=[]
    if result is None: return items
    texts=getattr(result,'txts',None); scores=getattr(result,'scores',None)
    if texts is not None:
        scores=scores if scores is not None else [0.5]*len(texts)
        return [(str(t),float(c)) for t,c in zip(texts,scores)]
    for row in result:
        if isinstance(row,(list,tuple)) and len(row)>=3: items.append((str(row[1]),float(row[2])))
    return items
...
def analyze_local(png:bytes,pdf_text:str,reader)->tuple[str,str,float,str,str]:
    ...
    for crop in crops:
        ...
        all_results.extend(_ocr_items(reader,gray))
    if folio=='REVIEW':
        header=image[0:int(h*.22),int(w*.45):w]
        header_results=_ocr_items(reader,header)
        folio=normalize_folio(' '.join(x[0] for x in header_results))
